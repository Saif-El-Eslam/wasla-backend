...
async function seedPlanCatalog() {
  await prisma.$executeRawUnsafe(`
    ...
    SELECT 'map-' || p."code" || '-' || f."key", p."id", f."id", true,
      ...
      END,
      CASE f."key"
        WHEN 'QR_BRANDING' THEN CASE p."code" WHEN 'FREE' THEN 'WASLA_SIGNED' WHEN 'MENU_STARTER' THEN 'WASLA_SIGNED' WHEN 'MENU_MULTI_BRANCH' THEN 'FULL_CUSTOM' WHEN 'WASLA_COMPLETE' THEN 'FULL_CUSTOM' ELSE 'VENUE_LOGO' END
        ELSE NULL
      END,
      CURRENT_TIMESTAMP
